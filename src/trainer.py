...
class Trainer:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            # if epoch > 5:
            #     # 超图训多了会出问题。
            #     self.model.hgats.requires_grad = False
            # if epoch>0 and epoch % 3== 0:
            #     print("Please check the emb of nodes for conditions")
            #     plot_tsne(self.model.embeddings['conditions'].weight, 'conditions-{}'.format(epoch))
            #     plot_tsne(self.model.node_emb_cond, 'conditions_hyper-{}'.format(epoch))
    ...
    def inference(self, dataloader, additional_outputs=None,
        ...
        for data in tqdm(dataloader, desc="Evaluation"):
            ...
            with torch.no_grad():
                ...
                # patient.append(output['patient'])
                #
                # f1_score, pred  = save_data(self.model, output['y_prob'], output['drugs'])
                # print(f1_score)
                # if 8<len(output['drugs'][0])<=12 and f1_score[-3]>=0.67:
                #     datas.append(data)
                #     preds.append(pred)
        ...
        # torch.save({'patient':torch.cat(patient,dim=0).cpu()}, '/home/czhaobo/HyperHealth/src/patient.pt')
        # torch.save({'data':datas, 'pred':preds}, '/home/czhaobo/HyperHealth/src/data_pred.pt')
    ...
    def evaluate(self, dataloader) -> Dict[str, float]:
        ...
        if self.model.mode is not None:
            y_true_all, y_prob_all, loss_mean, additional_outputs = self.inference(dataloader, additional_outputs=["global_prob", "local_prob"])
            global_prob, local_prob = additional_outputs['global_prob'], additional_outputs['local_prob']
            mode = self.model.mode
            metrics_fn = get_metrics_fn(mode)
            scores = metrics_fn(y_true_all, y_prob_all, metrics=self.metrics)
            scores_others = metrics_fn(y_true_all, global_prob, metrics=self.metrics, threshold=0.3)
            print("Additional threshold 0.3: ",scores_others)
            if global_prob is not None:
                scores["global_prob"] = metrics_fn(y_true_all, global_prob, metrics=self.metrics)
                scores["local_prob"] = metrics_fn(y_true_all, local_prob, metrics=self.metrics)
    ...
    def save_ckpt(self, ckpt_path: str) -> None:
        ...
        if 'kg_node_embedding.weight' in state_dict:
            del state_dict['kg_node_embedding.weight']
    ...
    def load_ckpt(self, ckpt_path: str) -> None:
        ...
        # state_dict['kg_node_embedding.weight'] = self.model.kg_node_embedding.weight
